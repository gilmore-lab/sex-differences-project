...
```{r}
# Yiming, please complete these...
male_typed_hobbies <- c("Home electronics", "Playing basketball", 
                        "Watching violent movies", "TV sports",
                        "Weight lifting")
```
...
```{r}
df.f <- df %>%
  dplyr::filter(., Sex.x == "Females") 
df.m <- df %>%
  dplyr::filter(., Sex.x == "Males") 
```

```{r}
rcorr(as.matrix(df[,c(20,31:51)]))$P # only video game is significant
rcorr(as.matrix(df.f[,c(20,31:51)]))$P # non
...
```{r}
rcorr(as.matrix(df[,c(29,31:51)]))$P # electronic,basketball, video games, violent movies
rcorr(as.matrix(df.f[,c(29,31:51)]))$P[,1] # electronic and basketball
rcorr(as.matrix(df.f[,c(29,31:51)]))$r[,1]
rcorr(as.matrix(df.m[,c(29,31:51)]))$P # fishing, basketball, TV sports
```

```{r}
rcorr(as.matrix(df.f[,c(29,31:51)]))$P # computers, weight lifting
```
