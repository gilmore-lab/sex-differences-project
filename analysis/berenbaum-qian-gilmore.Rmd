...
params:
  data_path: "csv"
  csv_fn: "combined_2021-08-19.csv"
...
# sex_diff_df <- readr::read_csv("~/OneDrive - The Pennsylvania State University/Box Migration Data/Project_Sex_difference_on_Motion_Perception/data/processed_data/combined_2021-04-30.csv")
```
...
sex_diff_df %>% 
  group_by(Sex) %>% 
  summarise(mean=mean(masscale,na.rm=T),sd=sd(masscale,na.rm=T))%>% 
  mutate_if(is.numeric, format, 2)
```
...
## Do some male-typed hobbies better correlate with contrast sensitivity than others?

Determine which hobbies are strongly and male-typed.

```{r}
hobby_new <- readr::read_csv("~/OneDrive - The Pennsylvania State University/Box Migration Data/Project_Sex_difference_on_Motion_Perception/Hobby_SPSS/hobby.csv")
```

```{r}
hobby_new$participant[1:23]<-c(119,110,121,123,125,127,320,112,114,129,324,116,131,118,133,120,330,135,122,124,137,139)
colnames(hobby_new)[2]<-"Participant"
df<-merge(sex_diff_df, hobby_new[,c(2,3, 15,20,25,31,32,36,37,38,39,42,45,47,48,49,53,54,56,58,62,71,72)], by="Participant")
```
...
#### Correlation of  male-typed hobbies and mental rotation

```{r}
#rcorr(as.matrix(df[,c(20,31:51)]))$P # only video game is significant
rcorr(as.matrix(df.f[,c(20,31:51)]))$P[,1] # non
#rcorr(as.matrix(df.m[,c(20,31:51)]))$P
```

#### Correlation of  male-typed hobbies and log contrast

```{r}
#rcorr(as.matrix(df[,c(28,31:51)]))$P # electronic,basketball, video games, violent movies
rcorr(as.matrix(df.f[,c(28,31:51)]))$P[,1] # electronic and basketball
rcorr(as.matrix(df.f[,c(28,31:51)]))$r[,1]
# rcorr(as.matrix(df.m[,c(28,31:51)]))$P # fishing, basketball, TV sports
```

#### Correlation of  male-typed hobbies and log motion

```{r}
#rcorr(as.matrix(df[,c(29,31:51)]))$P # electronics, surf, computer, video games, camping, weight lifting, work on cars, drag race
rcorr(as.matrix(df.f[,c(29,31:51)]))$P[,1] # computer, camping, weight lifting
rcorr(as.matrix(df.f[,c(29,31:51)]))$r[,1]
#rcorr(as.matrix(df.m[,c(29,31:51)]))$P # fishing, basketball, TV sports
```

#### get the CI for the male-type items which was found significant

```{r}
cor.test(
  df.f$log_contrast,
  df.f$playing_basketball,
  method = "pearson",
  alternative = "less"
)

es_mental_mas_contrast <- esci::estimateCorrelation.default(df.f, log_contrast, playing_basketball)
#es_mental_mas_contrast
```
```{r}
cor.test(
  df.f$log_contrast,
...
es_mental_mas_contrast <- esci::estimateCorrelation.default(df.f, log_contrast, electronics)
#es_mental_mas_contrast
```
...
es_mental_mas_contrast <- esci::estimateCorrelation.default(df.f, log_contrast, surf)
#es_mental_mas_contrast
```
```{r}
cor.test(
...
es_mental_mas_contrast <- esci::estimateCorrelation.default(df.f, log_contrast, Video_games)
#es_mental_mas_contrast
```
...
es_mental_mas_contrast <- esci::estimateCorrelation.default(df.f, log_contrast, violent_movies)
#es_mental_mas_contrast
```
...
cor.test(
  df.f$log_contrast,
  df.f$drinking_with_friends,
  method = "pearson",
  alternative = "less"
)

es_mental_mas_contrast <- esci::estimateCorrelation.default(df.f, log_contrast, drinking_with_friends)
#es_mental_mas_contrast
```