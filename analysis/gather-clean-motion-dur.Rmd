...
### Durations by trial and run

```{r motion-temporal-threshold-by-trial, include=FALSE}
s1_subset %>% ggplot(.) +
...
```{r motion-temporal-threshold-by-trial-2}
s1_subset %>% ggplot(.) +
...
s1_subset %>% ggplot(.) +
  ...
  geom_point() +
  geom_smooth(se = FALSE)  +
  ggtitle("Cumulative p(corr) by duration and run")
```
...
s1_subset %>%
  ...
  geom_point() +
  geom_smooth(se = FALSE) +
  ggtitle("RT by duration and run")
```
