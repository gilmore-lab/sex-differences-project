...
# combine male and female-type hobbies together
df1<-df1 %>%
           mutate(hobby=Masculine_hobbies-Feminine_hobbies) 
```
...
heatmap(x = corr$r, col = col, symm = TRUE)
```
...
lapply(df_complete_M[,2:3],FindOutliers)
lapply(df_complete_F[,2:3],FindOutliers)
which(df_complete_M$log_motion>-2)
df_complete_M <- df_complete_M[-which(df_complete_M$log_motion>-2),]
df_complete2<-rbind(df_complete_M, df_complete_F)
```
### Females
...
library(caret)
set.seed(123)
training.samples <- hobby$sex %>%
  createDataPartition(p = 0.8, list = FALSE)
train.data <- hobby[training.samples, ]
test.data <- hobby[-training.samples, ]
...
# Predicted probabilities of class memebership.  predictions$posterior
# Linear discriminants   predictions$x
```

<!-- The following code will cluster variables which have correlation coefficient greater than .95. -->
<!-- ```{r} -->
<!-- corDF=cor(hobby[,-1]); -->
...
```{r}
lda<-cbind(hobby_name,model$scaling)
# try to make prediction
lda<-lda %>%
  mutate(pred = ifelse(LD1 > 0.2, "m", ifelse(LD1 < -0.2, "f","na")))
```
