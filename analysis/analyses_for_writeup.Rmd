...
Detect outliers `r params$outlier_sd_thresh` SD from the grouped mean.
...
There were `r length(df2$motion_thr[outliers[['motion_thr']]])` outliers in the motion duration threshold measure, and `r length(df2$contrast_thr[outliers[['contrast_thr']]])` outliers in the contrast threshold measure.
...
## T-tests: Comparisons between sexes
...
Males took significantly shorter time to detect the direction of motion than females. 
...
Log transformed `contrast_thr` data show the predicted sex differences. 
Males detect the stimuli with lower contrast than females. 
