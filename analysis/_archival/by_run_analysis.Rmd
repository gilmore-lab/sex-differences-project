...
all_outliers <- lapply(df1, FindOutliers)
all_outliers
```
...
The outliers have higher visual acuity, so we ignore (include) them.
...
df1 %>% 
  ggplot(.) +
  aes(y = Threshold, x = Sex) +
  geom_violin() +
  facet_grid(Vision_task ~ .) +
  ggtitle('Vision thresholds by sex: All participants')
```

```{r}
df1[-all_outliers[['Threshold']],] %>%
  ggplot(.) +
  aes(y = Threshold, x = Sex) +
  geom_violin() +
  facet_grid(Vision_task ~ .) +
  ggtitle('Vision thresholds by sex: Excluding outliers')
```

By this criterion, we exclude `r length(a$Threshold)` runs from $n=$ `r length(unique(df1$Participant[a$Threshold]))` participants: `r unique(df1$Participant[a$Threshold])`.

```{r}
df2 <- df1[-all_outliers$Threshold,]
```
...
motion_df <- df2 %>%
  dplyr::filter(., Vision_task == 'motion')

contr_df <- df2 %>%
  dplyr::filter(., Vision_task == 'contrast')

this_var <- c("Vocabulary_score",
             "Mental_rot_score",
             "Feminine_hobbies",
             "Masculine_hobbies",
             "Contrast thresh",
             "Motion thresh")
sw_stat <- c(shapiro.test(df2$Vocabulary_score)['statistic'],
             ...
             shapiro.test(contr_df$Threshold)['statistic'],
             shapiro.test(motion_df$Threshold)['statistic']
             )

sw_p_val <- c(shapiro.test(df2$Vocabulary_score)['p.value'],
             ...
             shapiro.test(contr_df$Threshold)['p.value'],
             shapiro.test(motion_df$Threshold)['p.value']
             )
...
```{r}
shapiro.test(log(motion_df$Threshold))
hist(log(motion_df$Threshold))
```
...
```{r}
shapiro.test(log(contrast_df$Threshold))
hist(log(contrast_df$Threshold))
```
...
corr_df <- sex_diff_corr_summ("Motion_duration_Threshold",
                   "Contrast_Sensitivity_Threshold", print_table = FALSE)
...
corr_df <- sex_diff_corr_summ("Motion_duration_Threshold", 
                   "Mental_Rotation_scores", print_table = FALSE)
...
corr_df <- sex_diff_corr_summ("Motion_duration_Threshold", "Vocabulary_scores",
                   print_table = FALSE)
...
corr_df <- sex_diff_corr_summ("Contrast_Sensitivity_Threshold", "Mental_Rotation_scores",
                   print_table = FALSE)
...
corr_df <- sex_diff_corr_summ("Contrast_Sensitivity_Threshold", "Masculine_hobbies",
                              print_table = FALSE)
...
```{r}
corr_df <- sex_diff_corr_summ("Motion_duration_Threshold", "Feminine_hobbies",
                              print_table = FALSE)
