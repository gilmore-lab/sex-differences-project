...
```{r get-qualtrics}
active_survey_name <- "Sex difference survey REV 2019-11-11"
...
<!-- ```{r} -->
<!-- qualtrics_fns <- list.files(paste0(params$data_path, "/", params$qualtrics_csv_dir), pattern = "\\.csv$", full.names = TRUE) -->
...
<!-- demog_df <- demog_df %>% -->
<!--   dplyr::mutate(., p_id = stringr::str_remove_all(p_id, "[:-]")) %>% -->
<!--   dplyr::select(., p_id, gender, age_yrs) -->
...
These contrast file(s) are not matched by a motion file:

`r contrast_sub_ids[!(contrast_sub_ids %in% motion_sub_ids)]`

These motion files are not matched by a contrast file:

`r motion_sub_ids[!(motion_sub_ids %in% contrast_sub_ids)]`

The Qualtrics data have different 'suffixes', so we will compare them separately.
