...
1. Qualtrics survey data
2. Motion duration threshold task data.
3. Contrast sensitivity task data.
...
## Exclude scores based on pre-registration criteria
...
> We will exclude runs from the visual perception tasks with less than 50% accuracy (guess rate).

The marginal error is 1.97*sqrt(p(1-p)/30)=0.18
The 95 confidence interval for 50% correct are 50% +/- 18% [32% 68%]
...
Clean the thresholds which clearly have something wrong by the cumulative figures in <contrast-cum-perf-plots.html> and <motion-cum-perf-plots.html>.
...
### Median thresholds across runs by task

For one analysis pathway, we take the median threshold scores across the 4 runs.
...
df$race[which(df$race_2==1)] <- paste(df$race,"Black/African",sep=",")
df$race[which(df$race_3==1)] <- paste(df$race,"Caucasian",sep=",")
df$race[which(df$race_4==1)] <- paste(df$race,"Hispanic/Latino",sep=",")
df$race[which(df$race_5==1)] <- paste(df$race,"Native American",sep=",")
df$race[which(df$race_6==1)] <- paste(df$race,"Pacific Islander",sep=",")
df$race[which(df$race_7==1)] <- paste(df$race,"Prefer not to answer",sep=",")
...
### Data frame combining all run data

Create merged data frame with thresholds for all runs from all tasks.

Add "task" variable to `mdt` and `cst` frames since we are merging the fitted psychophysical function data.

```{r}
# mdt <- mdt %>%
#   dplyr::mutate(., task = 'motion')
# 
# cst <- cst %>%
#   dplyr::mutate(., task = 'contrast')
```
...
vis_df <- vis_df %>%
  rename(., 
         Sex = Gender,
         motion_run = run.x,
         motion_shape = shape.x,
         motion_scale = scale.x,
         motion_loglik = loglik.x,
         motion_thresh = threshold.x,
         motion_pcorr = percent_correct.x,
         contr_run = run.y,
         contr_shape = shape.y,
         contr_scale = scale.y,
         contr_loglik = loglik.y,
         contr_thresh = threshold.y,
         contr_pcorr = percent_correct.y) %>%
  dplyr::mutate(.,
...
all_df <- left_join(vis_df, qua_df, by = c("Participant", "Sex"))
```
...
```{r}
all_df$race <- ""
all_df$colorvision <- NA
all_df$race_5 <- as.numeric(all_df$race_5)
all_df$race[which(all_df$race_1==1)] <- "Asian"
all_df$race[which(all_df$race_2==1)] <- paste(all_df$race,"Black/African",sep=",")
all_df$race[which(all_df$race_3==1)] <- paste(all_df$race,"Caucasian",sep=",")
all_df$race[which(all_df$race_4==1)] <- paste(all_df$race,"Hispanic/Latino",sep=",")
all_df$race[which(all_df$race_5==1)] <- paste(all_df$race,"Native American",sep=",")
all_df$race[which(all_df$race_6==1)] <- paste(all_df$race,"Pacific Islander",sep=",")
all_df$race[which(all_df$race_7==1)] <- paste(all_df$race,"Prefer not to answer",sep=",")
# all_df$race[which(all_df$race_sum==2)] <- "Multiple"
which(all_df$color_vision1_4!=1)
which(all_df$color_vision1_5!=1) 
which(all_df$color_vision2_1!=4) 
which(all_df$color_vision2_2!=4) #4   9  11  64  73  92  95 102 129
which(all_df$color_vision2_3!=4)  #92
all_df$colorvision <- rowSums(all_df[,226:228],na.rm=TRUE)
```
...
```{r}
# save data
var <- c("Participant", "Gender","race","age","schoolyear","major","handedness","glasses","acuity","colorvision","stereo","mdt_median","cst_median","advoc_total_score","advoc_num_cor","MRT_cor","MRT_cor_both","hobfem","hobmas")
df1 <- df[,var]
colnames(df1)  <-  c("Participant", "Sex","Race","Age","School_year","Major","Handedness","Glasses","Acuity","Color_vision","Stereo","Motion_duration_Threshold","Contrast_Sensitivity_Threshold","Vocabulary_scores","Vocabulary_scores_cor_num","Mental_Rotation_scores","Mental_Rotation_scores_both","Feminine_hobbies","Masculine_hobbies")
...
out_df <- all_df %>%
  dplyr::select(., Participant,
                ...
                hobmas) %>%
  dplyr::rename(.,
                Vocab_num_corr = advoc_num_cor,
                Vocab_score = advoc_total_score,
                Mental_rot_score = MRT_cor,
                Mental_rot_both = MRT_cor_both,
                Feminine_hobbies = hobfem,
                Masculine_hobbies = hobmas)

readr::write_csv(out_df,
                 file.path(params$output_path,
                           "all-runs-2021-02-10.csv"))
```
