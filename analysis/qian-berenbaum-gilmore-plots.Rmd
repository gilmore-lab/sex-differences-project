...
```{r, eval = FALSE}
sex_diff_dotplot <- function(df = sex_diff_df,
                             x_var = "Sex",
                             y_var = "log_contrast",
                             axis_label = "log(contrast)") {
  x_var_s <- sym(x_var)
  ...
  geom_dotplot(inherit.aes = FALSE, binaxis = "y", stackdir = "center",
               aes(x = Sex,
                   y = !!y_var_s,
                   fill = Sex)) +
  theme.custom +
...
pp <- sex_diff_dotplot() + scale_color_brewer(palette = our_color_palette)
pp 
```
...
ggsave(
  plot = p_hist_vocab,
  filename = "paper_figs/fig-S02-vocab.jpg",
  units = "in",
  width = 3.4375,
  dpi = 600
)
