...
car::Anova(lm_mr_contr_motion_full)
confint(lm_mr_contr_motion_full)
lsr::etaSquared(lm_mr_contr_motion_full)
```

Drop interactions, but keep log_contrast, log_motion, and Sex.
...
Compare vision-only model with vision + Sex.

```{r}
anova(lm_mr_contr_motion_no_sex, lm_mr_contr_motion_no_interactions)
```

So, the RSS drops a bit, but not significantly. All things being equal, we choose the simpler model for parsimony reasons.
...
Finally, for completeness, we try one more model that includes the `Sex:log_motion` interaction indicated in the full model with interactions above.

```{r}
lm_mr_contr_motion_motion_sex_int <- lm(mental_rot ~ log_contrast + log_motion + log_motion:Sex, data = df_mr_contr_motion)
...
And we compare this to the model with vision measures alone but not `Sex`.

```{r}
anova(lm_mr_contr_motion_no_sex, lm_mr_contr_motion_motion_sex_int)
```

So, the best fitting model to predict mental rotation is the one *without* the `log_motion:Sex` interaction but *with* the vision measures.

Do we need `log_motion`?

```{r}
lm_mr_contr <- lm(mental_rot ~ log_contrast, data = df_mr_contr_motion)
summary(lm_mr_contr)
car::Anova(lm_mr_contr)
confint(lm_mr_contr)
lsr::etaSquared(lm_mr_contr)
```

```{r}
anova(lm_mr_contr, lm_mr_contr_motion_no_sex)
```

So, the best-fitting model overall is still the one with `log_contrast` and `log_motion` but not `Sex` or `Sex:log_motion`.
