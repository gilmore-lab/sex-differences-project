...
#### Model 3: `mental_rot ~ log_contrast + log_motion`

Let's drop `Sex`because its effect is small, and the `log_motion:Sex` interaction to see whether these improve the fit.
...
#### get scatterplot for stereo and contrast
```{r}
sex_diff_scatter3(
  df_trimmed,
  "log_contrast",
...
```{r}
df_factor_f <- sex_diff_df %>%
  filter(Sex=="Female") %>%
  dplyr::select(., Acuity,Stereo_amin,log_contrast, log_motion, mental_rot,vocab,     "Golf",
    "Home electronics",
    "Surfing",
    "Computers",
    "Video games",
    ...
    "Working on cars",
    "Drag racing") %>%
  filter(!is.na(log_contrast),!is.na(log_motion),!is.na(Stereo_amin))
df_factor_f$Acuity<-as.numeric(factor(df_factor_f$Acuity,levels=c("20/16","20/20","20/25","20/32"),labels=c(1,2,3,4)))
df_factor_f$Stereo_amin<-as.numeric(factor(df_factor_f$Stereo_amin,levels=c("40","50","60","80","100","140","400","800"),labels=c(1,2,3,4,5,6,7,8)))
```

```{r}
df_factor_m <- sex_diff_df %>%
  filter(Sex=="Male") %>%
  dplyr::select(., Acuity,Stereo_amin,log_contrast, log_motion, mental_rot,vocab,     "Golf",
    "Home electronics",
    "Surfing",
    ...
    "Drag racing") %>%
  filter(!is.na(log_contrast),!is.na(log_motion),!is.na(Stereo_amin))
df_factor_m$Acuity<-as.numeric(factor(df_factor_m$Acuity,levels=c("20/16","20/20","20/25","20/32"),labels=c(1,2,3,4)))
df_factor_m$Stereo_amin<-as.numeric(factor(df_factor_m$Stereo_amin,levels=c("40","50","60","80","100","140","400","800"),labels=c(1,2,3,4,5,6,7,8)))
```

```{r}
df.fa.m <- factanal(df_factor_m, factors = 2)
df.fa.m
```

```{r}
df.fa.f <- factanal(df_factor_f, factors = 2)
df.fa.f
```

```{r}
df_factor <- sex_diff_df %>%
  dplyr::select(., Sex,Acuity,Stereo_amin,log_contrast, log_motion, mental_rot,vocab,     "Golf",
    "Home electronics",
    "Surfing",
    "Computers",
    "Video games",
    ...
    "Drag racing") %>%
  filter(!is.na(log_contrast),!is.na(log_motion),!is.na(Stereo_amin))
df_factor$Sex<-as.numeric(factor(df_factor$Sex,levels=c("Male","Female"),labels=c(1,0)))
df_factor$Acuity<-as.numeric(factor(df_factor$Acuity,levels=c("20/16","20/20","20/25","20/32"),labels=c(1,2,3,4)))
df_factor$Stereo_amin<-as.numeric(factor(df_factor$Stereo_amin,levels=c("40","50","60","80","100","140","400","800"),labels=c(1,2,3,4,5,6,7,8)))
```


```{r}
df.fa <- factanal(df_factor, factors = 2)
df.fa
```

```{r}
df.fa3 <- factanal(df_factor, factors = 3)
df.fa3
```

```{r}
# varimax
plot(df.fa$loadings[,1], 
     df.fa$loadings[,2],
     xlab = "Factor 1", 
     ylab = "Factor 2", 
     ylim = c(-1,1),
...
text(df.fa$loadings[,1]-0.08, 
     df.fa$loadings[,2]+0.08,
      colnames(df_factor),
      col="blue")
abline(h = 0, v = 0)
```
...
```{r}
df_factor_all <- sex_diff_df[,c(2,9,11,18,20,27:88)]  %>%
  filter(!is.na(log_contrast),!is.na(log_motion),!is.na(Stereo_amin))
df_factor_all$Sex<-as.numeric(factor(df_factor_all$Sex,levels=c("Male","Female"),labels=c(1,0)))
df_factor_all$Acuity<-as.numeric(factor(df_factor_all$Acuity,levels=c("20/16","20/20","20/25","20/32"),labels=c(1,2,3,4)))
df_factor_all$Stereo_amin<-as.numeric(factor(df_factor_all$Stereo_amin,levels=c("40","50","60","80","100","140","400","800"),labels=c(1,2,3,4,5,6,7,8)))
```

```{r}
df.fa2 <- factanal(df_factor_all, factors = 3)
df.fa2
```

## Check internal reliability of female-typed hobbies 

Determine which hobbies are strongly female-typed according to (Lippa, 1992):
...
female_hobbies <- sex_diff_df[, varf]
female_hobbies_F <- female_hobbies %>% 
  dplyr::filter(Sex == "Female")
```
