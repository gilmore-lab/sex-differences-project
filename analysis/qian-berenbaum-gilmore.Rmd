---
title: "Analyses for Qian, Berenbaum, & Gilmore"
...
One models `mental rotation ~ contrast + sex + contrast:sex` and determines whether adding the `sex` term provides a better fit relative to `mental rotation ~ contrast` alone. 
The other looks for sex differences in the residuals of the models `mental rotation ~ contrast` and `mental rotation ~ motion`.

We take the model-based approach here.
...
We choose the simpler model for parsimony reasons and a reduction in RSS even though the fit does not notably improve.

Drop `Sex` because its effect is small.

```{r}
lm_mr_contr_motion_no_sex <- lm(mental_rot ~ log_contrast + log_motion + log_motion:Sex, data = df_mr_contr_motion)
...
Compare to fuller model.

```{r}
anova(lm_mr_contr_motion_motion_sex_int, lm_mr_contr_motion_no_sex)
```

The model without `Sex` fits better, and we note that the `log_motion:Sex` interaction is small. Let's test a model without that interaction

```{r}
lm_mr_contr_motion_no_interactions <- lm(mental_rot ~ log_contrast + log_motion, data = df_mr_contr_motion)
...
The fit does not improve substantially, but we prefer the simpler model with a smaller RSS and fewer parameters.

For completeness, let's compare this final model to two alternatives, one with sex alone.

```{r}
lm_mr_only_sex <- lm(mental_rot ~ Sex, data = df_mr_contr_motion)
summary(lm_mr_only_sex)
car::Anova(lm_mr_only_sex)
confint(lm_mr_only_sex)
lsr::etaSquared(lm_mr_only_sex)
```

Comparing this to one with vision but not sex.

```{r}
anova(lm_mr_contr_motion_no_interactions, lm_mr_only_sex)
```

The model with the smaller RSS is the one with the vision variables included. The RSS is smaller, and it fits better than a model with `Sex` alone.

Let's also compare a model with `Sex` and the vision measures to one with only the vision measures.

```{r}
lm_mr_contr_motion_sex_no_int <- lm(mental_rot ~ log_contrast + log_motion + Sex, data = df_mr_contr_motion)
summary(lm_mr_contr_motion_sex_no_int)
car::Anova(lm_mr_contr_motion_sex_no_int)
confint(lm_mr_contr_motion_sex_no_int)
lsr::etaSquared(lm_mr_contr_motion_sex_no_int)
```

```{r}
anova(lm_mr_contr_motion_sex_no_int, lm_mr_contr_motion_no_interactions)
```

The addition of `Sex` does not improve the fit of the model with vision measures alone.

Do we need `log_motion`?

```{r}
lm_mr_contr_no_motion <- lm(mental_rot ~ log_contrast, data = df_mr_contr_motion)
summary(lm_mr_contr_no_motion)
car::Anova(lm_mr_contr_no_motion)
confint(lm_mr_contr_no_motion)
lsr::etaSquared(lm_mr_contr_no_motion)
```

```{r}
anova(lm_mr_contr_motion_no_interactions, lm_mr_contr_no_motion)
```

Yes, we do. The more complex model with `log_motion` fits better.

So, the best fitting and most parsimonious model includes both vision measures, but not sex.
...
```{r}
anova(lm_mr_contr_motion_no_interactions, lm_mr_contr_motion_no_sex)
```

So, the RSS drops a bit, but not significantly. All things being equal, we still choose the simpler model for parsimony reasons.

```{r}
lm_mr_sex <- lm(mental_rot ~ Sex, data = df_mr_contr_motion)
summary(lm_mr_contr_motion_no_sex)
car::Anova(lm_mr_contr_motion_no_sex)
confint(lm_mr_contr_motion_no_sex)
lsr::etaSquared(lm_mr_contr_motion_no_sex)
...
```{r}
anova(lm_mr_contr_motion_no_sex, lm_mr_sex)
```

The more complex model with both vision measures fits better than one with sex alone.

Even excluding the two outliers in motion duration thresholds, we still conclude that vision measures are a better predictor of individual differences in mental rotation scores than sex.

## Do men and women differ in other ways?

The samples do not appear substantially different in other measures.
