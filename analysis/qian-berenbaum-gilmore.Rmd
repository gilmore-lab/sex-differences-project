...
One can model `mental rotation ~ contrast + motion + sex` and determines whether adding the `sex` term provides a better fit relative to `mental rotation ~ contrast` alone.
...
Let's drop `Sex` because its effect is small.
...
Compare this to the fuller model.
