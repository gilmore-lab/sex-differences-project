...
```{r}
df$glasses <- factor(df$glasses, labels = c("glasses/contacts", "none"))
```
...
```{r}
df$age <- factor(df$age, labels = c("18-19", "20-21", "22-23", "23+"))
```
...
```{r}
df$schoolyear <- factor(df$schoolyear, labels = c("1st", "2nd", "3rd", "4th"),
                        ordered = TRUE)
```
...
`What is your preferred hand to press the arrow keys?`

```{r}
df$handedness <- factor(df$handedness, labels = c("left", "right", "either"))
```

### Visual acuity

10/80,10/63,10/50,10/40,10/32,10/25,10/20,10/16,10/12.5,10/10,10/8
...
```{r}
# Scores range from 18-21; but is 21 10/8 (20/16) or 10/10 (20/20)
df$acuity <- factor(df$acuity, labels = c("20/32", "20/25", "20/20", "20/16"))
```

### Stereo

The levels are in arcmin:

800, 400, 200, 140, 100, 80, 60, 50, 40

```{r}
df$stereo <- factor(df$stereo, labels = c(800, 400, 140, 100, 80, 60, 50, 40))
```
