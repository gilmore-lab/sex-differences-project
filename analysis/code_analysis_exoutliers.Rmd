...
```{r}
sex_diff_scatter <- function(df, x_var, y_var, x_rev = FALSE, y_rev = FALSE, an1,an2, anx,any) {
  require(tidyverse)
  require(ggExtra)
  ...
  theme(legend.position="bottom", legend.title=element_blank())+ 
    annotate("text", x = anx, y = any, label = c(an1,an2),color=c('red','darkturquoise'))
  
  # Reverse scale for threshold measures?
  if (x_rev) p <- p + scale_x_reverse()
  if (y_rev) p <- p + scale_y_reverse()

p1 <- ggExtra::ggMarginal(p,groupColour = TRUE, groupFill = TRUE)
p1    
}
...
sex_diff_scatter(df_complete, "Motion_duration_Threshold", "Contrast_Sensitivity_Threshold",
                 x_rev = TRUE, y_rev = TRUE,"r = 0.22*", "r=0.54***", c(0.16), c(0.16,0.24))
...
sex_diff_scatter(df_complete, "Motion_duration_Threshold", 
                 "Mental_Rotation_scores", x_rev = TRUE,y_rev = FALSE,"r = 0.31**", "r=0.09", c(0.17), c(24,33))
...
```{r}
sex_diff_scatter(df_complete, "Motion_duration_Threshold", 
                 "Vocabulary_scores", x_rev = TRUE,y_rev = FALSE,"r = 0.18", "r=0.22", c(0.17), c(24,33))
...
sex_diff_scatter(df_complete, "Contrast_Sensitivity_Threshold", 
                 "Mental_Rotation_scores", x_rev = TRUE,y_rev = FALSE,"r = -0.19*", "r=0.12", c(0.24), c(19,32))
...
```{r}
sex_diff_scatter(df_complete, "Contrast_Sensitivity_Threshold", 
                 "Masculine_hobbies", x_rev = TRUE,y_rev = FALSE,"r = -0.19*", "r=0.26", c(0.24), c(2.4,4))
...
```{r}
sex_diff_scatter(df_complete, "Motion_duration_Threshold", 
                 "Feminine_hobbies", x_rev = TRUE,y_rev = FALSE,"r = 0.17*", "r=-0.01", c(0.165), c(4,3))
...
```{r}
# mental rotations
cor.test(formula = ~ Mental_Rotation_scores + Motion_duration_Threshold,
         data = df_corr,
         subset = Sex == "Females", alternative="less")
cor.test(formula = ~ Mental_Rotation_scores + Contrast_Sensitivity_Threshold,
         data = df_corr,
         subset = Sex == "Females", alternative="less")
cor.test(formula = ~ Mental_Rotation_scores + Motion_duration_Threshold,
         data = df_corr,
         subset = Sex == "Males", alternative="less")
cor.test(formula = ~ Mental_Rotation_scores + Contrast_Sensitivity_Threshold,
         data = df_corr,
         subset = Sex == "Males", alternative="less")
# vocabulary
cor.test(formula = ~ Vocabulary_scores + Motion_duration_Threshold,
         data = df_corr,
         subset = Sex == "Females", alternative="less")
cor.test(formula = ~ Vocabulary_scores + Contrast_Sensitivity_Threshold,
         data = df_corr,
         subset = Sex == "Females", alternative="less")
cor.test(formula = ~ Vocabulary_scores + Motion_duration_Threshold,
         data = df_corr,
...
cor.test(formula = ~ Vocabulary_scores + Contrast_Sensitivity_Threshold,
         ...
         subset = Sex == "Males", alternative="less")
#hobbies
cor.test(formula = ~ Masculine_hobbies    + Contrast_Sensitivity_Threshold,
         data = df_corr,
         subset = Sex == "Females", alternative="less")
cor.test(formula = ~ Feminine_hobbies    + Motion_duration_Threshold,
         data = df_corr,
         subset = Sex == "Females", alternative="greater")
cor.test(formula = ~ Masculine_hobbies    + Contrast_Sensitivity_Threshold,
         data = df_corr,
         subset = Sex == "Males", alternative="greater")
cor.test(formula = ~ Feminine_hobbies    + Motion_duration_Threshold,
         data = df_corr,
         subset = Sex == "Males", alternative="less")
# two vision tests
cor.test(formula = ~ Motion_duration_Threshold    + Contrast_Sensitivity_Threshold,
         data = df_corr,
         subset = Sex == "Females", alternative="greater")
cor.test(formula = ~ Motion_duration_Threshold    + Contrast_Sensitivity_Threshold,
         data = df_corr,
