...
compute_within_p_var <- function(df) {
  ...
  med_cor <- median(sort(c(cor_vals[1,2], 
                    ...
                    cor_vals[2,4], 
                    cor_vals[3,4])))
  data.frame(tr30_sd = tr30_sd, tr30_rng = tr30_rng,
...
compute_motion_var_measures <- function(fn) {
  ...
  var_m_df$sub_id <- stringr::str_extract(fn, "[0-9]+")
  var_m_df
}
```
...
motion_var_df <- purrr::map_dfr(motion_fl, compute_motion_var_measures)
```
